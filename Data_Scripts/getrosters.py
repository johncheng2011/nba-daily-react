...
for team in teams:
    ...
    sql += str(num_to_team[team[1]]) + "_Roster"+season_split+" (teamid INT, playerid INT, playername VARCHAR(255), num INT, position VARCHAR(255), height VARCHAR(255), weight INT, age INT, PRIMARY KEY (playerid))"
    cursor.execute(sql)
    print(num_to_team[team[1]])

for roster in rosters:
    pruneSQL = "DELETE FROM "+ num_to_team[roster[0][0]] +"_Roster"+season_split+" WHERE NOT("
    for player in roster:
        sql = "INSERT INTO " + num_to_team[player[0]] + "_Roster"+season_split+" (teamid,playerid,playername,num,position,height,weight,age) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)"
